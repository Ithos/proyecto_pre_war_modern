...
const SearchResults = ({ cardSearchObject, bannedCards, cardsPerPage, scrollPosition }) => {
    ...
    const bannedCardsString = JSON.stringify(bannedCards);
    const memoizedBannedCards = useMemo(() => { return bannedCardsString; }, [bannedCardsString]);

    const [currentPage, setCurrentPage] = useState(1);
    const [pageUrl, setPageUrl] = useState(cardSearchObject.hasOwnProperty('next_page') ? cardSearchObject.next_page : null);
    const [filteredCards, setFilteredCards] = useState([...cardSearchObject.data.filter((card) => !memoizedBannedCards.includes(card.name))]);
    const [isLoading, setIsLoading] = useState(true);

    const maxPage = Math.ceil(cardSearchObject.total_cards / cardsPerPage);

    const adjustPage = useCallback( async function () {
        setIsLoading(true);
        if (pageUrl) {
            ...
            if (currentPageMatch && currentPage > 0 && currentPage <= maxPage) {
                const tmpPage = pageUrl.replace(/&page=\d+/, `&page=${currentPage}`)
                    ...
                    setFilteredCards([...result.data.filter((card) => !memoizedBannedCards.includes(card.name))]);
            }
        }
        setIsLoading(false);
    } , [currentPage, maxPage, pageUrl, memoizedBannedCards]);
    ...
    return (
        <section className="searchResults" id="search-results">
            <section className="resultsHeader">
                <span> Page {currentPage} of {maxPage} &#9148; {maxPage > 1 ? cardSearchObject.total_cards : filteredCards.length - filteredCards.filter((card) => memoizedBannedCards.includes(card.name)).length} results found</span>
            ...
            { 
                !isLoading && filteredCards.length > 0 && filteredCards.map( (card) => (
            ...
            { filteredCards.length === 0 && !isLoading && <div className="no-results">No results found</div> }
            { isLoading && <section>
